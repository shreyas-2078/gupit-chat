import { $ as safeGet, A as Sha1, B as getDefaultEmulatorHostnameAndPort, C as LogLevel, D as Provider, E as ComponentContainer, F as contains, G as isMobileCordova, H as isAdmin, I as createMockUserToken, J as isValidFormat, K as isNodeSdk, L as deepCopy, M as assertionError, N as base64, O as Deferred, P as base64Encode, Q as querystring, R as deepEqual, T as Component, U as isCloudWorkstation, V as getModularInstance, W as isEmpty, X as map, Y as jsonEval, Z as pingServer, c as _getProvider, et as stringLength, f as _registerComponent, g as getApp, j as assert, n as SDK_VERSION$1, nt as stringify, q as isReactNative, tt as stringToByteArray, u as _isFirebaseServerApp, w as Logger, x as registerVersion, z as errorPrefix } from "./index.esm-DI7TFzKK.js";
	...
	*/
	remove(key, comparator) {
		...
		if (comparator(key, n.key) < 0) {
			...
			if (comparator(key, n.key) === 0) if (n.right.isEmpty()) return SortedMap.EMPTY_NODE;
			else {
...
var buildChildSet = function(childList, cmp, keyFn, mapSortFn) {
	...
	const buildFrom12Array = function(base12) {
		...
		const buildPennant = function(chunkSize, color) {
			...
			attachPennant(new LLRBNode(keyFn ? keyFn(namedNode) : namedNode, namedNode.node, color, null, childTree));
		};
...
var View = class {
	constructor(query_, initialViewCache) {
		...
		this.viewCache_ = newViewCache(new CacheNode(eventSnap, initialEventCache.isFullyInitialized(), filter.filtersNodes()), newServerCache);
...
function repoStartTransaction(repo, path, transactionUpdate, onComplete, unwatcher, applyLocally) {
	...
	if (newVal === void 0) {
		...
		} else priorityForNode = (syncTreeCalcCompleteEventCache(repo.serverSyncTree_, path) || ChildrenNode.EMPTY_NODE).getPriority().val();
		const serverValues = repoGenerateServerValues(repo);
...
function repoManagerApplyEmulatorSettings(repo, hostAndPort, emulatorOptions, tokenProvider) {
	...
	repo.repoInfo_ = new RepoInfo(hostAndPort, isCloudWorkstation(hostAndPort.substring(0, portIndex)), repo.repoInfo_.namespace, repo.repoInfo_.webSocketOnly, repo.repoInfo_.nodeAdmin, repo.repoInfo_.persistenceKey, repo.repoInfo_.includeNamespaceInQueryParams, true, emulatorOptions);
...
function connectDatabaseEmulator(db, host, port, options = {}) {
	...
	if (db._instanceStarted) {
		if (hostAndPort === db._repoInternal.repoInfo_.host && deepEqual(options, repo.repoInfo_.emulatorOptions)) return;
	...
	if (isCloudWorkstation(host)) pingServer(host);
...
*/
function runTransaction(ref, transactionUpdate, options) {
	ref = getModularInstance(ref);
...
function _initStandalone({ app, url, version, customAuthImpl, customAppCheckImpl, nodeAdmin = false }) {
	...
	*/
	...
	if (customAppCheckImpl) {
		...
		appCheckProvider.setComponent(new Component("app-check-internal", () => customAppCheckImpl, "PRIVATE"));
	}
